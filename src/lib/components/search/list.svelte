<script>
  ...
  const setSearch = (name, lat, lon) => {
    goto(`?location=${name}&lat=${lat}&lon=${lon}`, {
      replaceState: true,
      keepfocus: true,
    });
    searchOpen.set(true);
...
<style lang="postcss">
  ul {
    @apply w-full py-4 mx-auto space-y-4 overflow-y-auto text-center;

    li {
      @apply flex items-center justify-between w-full p-4 mx-auto text-left rounded-md cursor-pointer border-[1px] border-secondary group;

      button {
        @apply grid w-12 h-12 p-0 duration-100 bg-black rounded-md place-items-center group-hover:bg-primary-500;
      }
    }
  }
</style>
