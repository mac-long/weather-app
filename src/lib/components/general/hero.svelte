<script>
	...
	const handleClick = async () => {
		if ($ai.open) {
			...
			const res = await fetch(`/api/ai?type=Sunshine&location=London&temperature=21&unit=c`).then(
				(res) => res.json()
			);
			ai.set({
				open: true,
...
>
	<div class="flex flex-col items-center justify-center text-center text-white relative">
		...
		{#if $page.url.pathname === '/'}
			<button class="absolute bottom-0 right-0 w-auto mr-4 mb-[1px] ghost" on:click={handleClick}>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					fill="currentColor"
					class="bi bi-robot w-8 h-8 m-0"
					viewBox="0 0 16 16"
				>
					<path
			...
			{#if $ai.open}
				...
				<div class="absolute bottom-8 right-10 font-bold w-64 h-24 bg-white rounded-lg shadow-lg">
					<button
						class="absolute top-0 right-0 text-secondary hover:text-red-400 text-2xl mr-1 bg-transparent p-0 hover:bg-transparent w-auto h-auto"
